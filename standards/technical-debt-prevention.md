...
- **Under review, collapse rather than annotate.** Instruction text accrues clauses: each review finding gets fixed by adding one, and the added clause becomes the next cycle's finding — usually in the shape "two places state this rule and they disagree." When a cycle's findings cluster into that shape, stop patching and do a collapse pass: pick the one location that owns the rule, state it there, and reduce every other mention to a pointer or delete it. Prefer a general rule over an enumeration of the cases it covers — enumerations go stale as cases are added, and they are what duplicate-disagreement findings attach to. (This governs prose describing rules; a partition over enum variants in code still has to be exhaustive — see `/quality-review` Step 5.) A fix that grows the file is not wrong, but it is the cue to check whether the clause you just added restates something already stated elsewhere.
