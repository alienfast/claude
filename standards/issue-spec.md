...
- **Who applies it:** `/prd` (single-issue runs on create; batches after collision edges wire), `/spec` (after research + interview + explicit signoff), `/reflect`'s filing script (its proposal bodies already carry problem/outcome/criteria), `/quality-review`'s deferred-item filing — for severity-carrying items (pre-existing Critical/High/Medium findings, routed by its Step 5 fix-loop lane split, by its Step 5 auto-mode ceiling resolution (Medium survivors only), or by its Step 6 sub-step 5 re-review) and for evidence-complete deferred items (no open design choice; criteria stated testably from measurements the filing run itself produced — its Step 6 sub-step 6 label rule), whose bodies it composes in the certifiable problem/outcome/criteria shape — or a human in Linear when they judge a spec complete.
...
- `/quality-review` — files deferred items, certifying the severity-carrying ones (Critical/High/Medium) and the evidence-complete ones (no open design choice; criteria testable from the run's own measurements), whose bodies it composes in the certifiable problem/outcome/criteria shape
