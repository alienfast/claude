...
- **Adding a new `/start`-terminal tag? Do NOT wire it into `hooks/full-continue.sh`'s close set** — see the *Historical (recovered-from) tags* note above. A `/full` window is closed by exactly two things: a `/finish`-completion tag (`RELEASED` / `SHIPPED-MERGE` / `SHIPPED-PR` / `DEFERRED-MERGE`) or a **new user-typed** `/start`/`/full` command (which also supersedes a crashed, tagless `/full`). A `/start`-terminal tag (`BLOCKED-ON-REVIEW`, `CANCELED`, `ABANDONED`, `SKIPPED-BLOCKED`, `INTERACTIVE-READY`) must **never** close a still-in-flight window: a run can be *resumed* past one, leaving it in transcript history while the `/full` continues, so treating it as a close silently disables the handoff (the BF-391 regression). The hook's recognizer therefore lists only `READY-FOR-FINISH` (the auto-continue fire trigger) plus that `/finish`-completion close set; every `/start`-terminal tag and mid-flight `IN-PROGRESS` is deliberately absent. Any change to the hook must keep `hooks/full-continue.test.sh` green.
...
- `~/.claude/hooks/full-continue.sh` — the terminal-tag *consumer*: auto-continues the `/full` macro by firing only on `READY-FOR-FINISH`. A `/full` window is closed only by a `/finish`-completion tag (RELEASED / SHIPPED-MERGE / SHIPPED-PR / DEFERRED-MERGE) or a new user-typed `/start`/`/full` command — never by a `/start`-terminal tag (see the *Historical (recovered-from) tags* note and the Rules note above). Regression-guarded by `hooks/full-continue.test.sh`.
