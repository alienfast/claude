...
| `INTERACTIVE-READY` | `/start` Step 0 sub-step 4 (`interactive` token) | Worktree set up + issue claimed (In Progress), baseline green, session entered into the worktree; `/start` stopped before planning/implementing/reviewing — the user drives from here | work in the worktree, then `/finish <id> merge` (optionally `/quality-review` first) |
...
INTERACTIVE-READY: PL-318 — worktree ready at .claude/worktrees/pl-318 (branch rosskevin/pl-318-foo), claimed In Progress, baseline green. Work here; run /finish PL-318 merge when done (or /quality-review PL-318 first).
