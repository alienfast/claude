...
| `AUTO-CONTINUE` | `/auto` | One loop iteration finished (issue shipped, or skipped/failed below the circuit-breaker threshold), or paused on a transient Claude-API failure (mid-issue or pre-pick; retry pending); the next `/loop` iteration proceeds/resumes. A targeted run (`/auto <ISSUE-ID>`) ends with the same tag and a `targeted run complete.` trailer — one-shot, no wakeup follows | none — the loop continues (or, targeted, the run is over) |
...
- `~/.claude/hooks/full-continue.sh` — the terminal-tag *consumer*: auto-continues the `/full` macro by firing on `READY-FOR-FINISH` and — one step upstream — on a passing `/quality-review` verdict returned inside an in-flight `/start` before the tag is emitted (BF-392). A `/full` window is closed only by a `/finish`-completion tag (RELEASED / SHIPPED-MERGE / SHIPPED-PR / DEFERRED-MERGE) or a new user-typed `/start`/`/full` command — never by a `/start`-terminal tag (see the Rules note above). Regression-guarded by `hooks/full-continue.test.sh`.
