...
- **Adding a new macro-terminal tag? Wire it into `hooks/full-continue.sh`.** When a new tag can end a `/full` run — especially a `/start`-family exit that terminates *without* a `/finish` dispatch (`CANCELED`, `ABANDONED`, `SKIPPED-BLOCKED`, `INTERACTIVE-READY`) — add it to the hook's tag recognizer AND both close-set regexes so it closes a dangling `/full` window; otherwise a later `READY-FOR-FINISH` can inherit a crashed `/full`'s still-open window and be force-dispatched into a stale `/finish`. The recognizer must list it, since the close sets only filter already-recognized tags. Two deliberate exclusions: `READY-FOR-FINISH` (the auto-continue *fire trigger*, never a close member) and mid-flight `IN-PROGRESS` (a pause, not terminal). The FIRE gate stays `READY-FOR-FINISH`-only regardless, so a non-`READY-FOR-FINISH` tag never itself dispatches `/finish` — adding a genuinely terminal tag to the close set is safe by construction (it can only close an already-dead window).
...
- `~/.claude/hooks/full-continue.sh` — the terminal-tag *consumer*: auto-continues the `/full` macro by firing only on `READY-FOR-FINISH`, and treats the macro-closing tags (RELEASED / SHIPPED-MERGE / SHIPPED-PR / DEFERRED-MERGE / BLOCKED-ON-REVIEW / CANCELED / ABANDONED / SKIPPED-BLOCKED / INTERACTIVE-READY) as its window-close set. See the Rules note above on wiring a new terminal tag into it.
