...
### A self-claim does not hide an issue — it promotes it

That hiding is asymmetric, deliberately: `next-candidates.sh` drops issues assigned to **someone else** and keeps issues assigned to **you**, at tier 1 — the top tier (its candidate filter's own comment reads "Assigned-to-me stays (that is tier 1)"). Two consequences follow, and both are routinely read the wrong way round:

- **The `hidden as claimed by a person` note is not a check that your claim landed.** Its count selects `assignee != $me`, so a self-claim never moves it. Confirm a claim by reading `.assignee` back — see [skills/linear/SKILL.md](../skills/linear/SKILL.md) gotcha #8, where the write can report success without landing — never by watching that number.
- **Where sessions share one Linear viewer identity — every fleet on one machine — claiming an issue does not protect it from siblings; it puts it first in their ranking.** The reachable case is re-grooming an already-`specified` issue: the claim is held for a whole interview while the issue stays pickable and now outranks the pool. Use a control that actually hides it (`needs decision` while the question is open, or work it outside the fleet window), and treat a claim as what it is — a signal to other *people*, not a lock against other *sessions*.

Note which half of a claim does the hiding when one is also writing a state: for a fleet it is the **state**, since `WORKABLE_STATES` is Backlog/Planned/Todo and In Progress is not among them. A claim whose assignee landed but whose state did not therefore looks claimed to a human while staying fully pickable to every session — which is why both halves are verified rather than inferred from the write's exit status ([scripts/linear-claim.sh](../scripts/linear-claim.sh)).
...
## A batch approval covers the presented set, not a selector

When a set of Linear writes — state moves, label changes, cancellations, promotions — is presented for one approval, the approval attaches to exactly the ids shown. Hand the applier that frozen list; never a selector ("every cancel proposal", "every Backlog issue on the chain") re-evaluated at apply time, and never a re-derivation of the rows. Both directions have been measured: a selector picks up what arrived after the presentation — on 2026-09-22 a background `/triage scan` wrote four more cancel proposals between an 18-issue presentation and its apply, and a verdict-keyed applier canceled 22, four of them unapproved and reverted; a re-derivation drops what the presentation had — on 2026-08-13 `/auto-prep`'s rows named 21 chain members and a roots-only recomputation promoted 12. Each applier takes the ids as an argument (`skills/triage/scripts/triage-apply.sh --ids`; `/auto-prep`'s PROMOTE-SET takes the FOCUS-ROOT rows) and skips an id whose record no longer matches rather than widening to whatever matches now.
