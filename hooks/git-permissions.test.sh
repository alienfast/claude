...
echo "== 7. checkout stays blocked on every operand form — branch and path are lexically identical"
...
assert_blocked "git checkout main" "indistinguishable from a path; use git switch"
assert_blocked "git checkout main -- ."

echo "== 7b. switch is branch-only (no pathspec form), so it is the allowed way to change branch"
assert_allowed "git switch --detach"
assert_allowed "git switch -c feature/x"
assert_allowed "git switch main" "cannot destroy a file; git refuses a clobbering switch"
assert_allowed "git switch bjones84arrow/ja-382-pricing" "slashed branch names are branches, not paths"
assert_allowed "git switch -" "previous branch"
assert_blocked "git restore foo.ts" "the destructive half of old checkout still blocked"
