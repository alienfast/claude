...
assert_blocked_in "$R" "git checkout feature" "a running /fleet-sequence"
printf '{"status": "done", "queue": ["TT-1"]}\n' > "$R/tmp/fleet-sequence.json"
assert_allowed_in "$R" "git checkout feature" "a finished sequence"
