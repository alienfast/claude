...
- **Positional field tools neither understand CSV quoting nor check your index — and both failures print plausible output and exit 0.** `cut -d,` / `awk -F,` split on every comma, so (a) a wrong field number silently returns a *neighbouring* column, and (b) a comma inside a quoted field shifts every later field by the cumulative number of commas in the quoted fields preceding it — so the same command can return different columns on different rows of one file. Measured on `doc/groups/GROUP_EMAILS_ROLE_PREFIX_2026-08-06.csv` (16 columns, 46 data rows) while planning BF-989: `cut -d, -f15` was run to survey `suggested_classification`, which is field **14** — so on the 42 rows with no embedded comma before that point it returned `suggested_decision` by plain off-by-one, and on the 4 rows whose earlier quoted fields did carry commas it returned `entity_kind`, `org_id`, or — twice — the intended column after all. The blended output (`41 confirm, 1 hold, 1 private, 2 shared`) reads like a clean survey of one column and is a survey of three. That reading became "neither committed artifact records a `classification:` segment", posted in a Linear plan comment; `grep -c '| classification: '` over the whole file returns **43** of 46. Confirm the field number against the header before trusting a positional extract, and prefer a whole-file count on a pattern that cannot shift (`grep -c`) or a real parser (`ruby -rcsv -e`) — neither depends on field position at all.
- **A `grep` for a multi-word phrase misses it when a line wrap falls inside, and the empty result reads as proof the text is gone.** Matching is line-oriented, so this failure *inverts* the answer rather than degrading it: an empty result is exactly what a genuinely-absent string produces, so "already fixed", "already deleted", "the file makes no such claim" each come back falsely confirmed. The check that most often takes this shape — *did a later fix batch already rewrite this?* — is one `/quality-review` and `/reflect` run every session. Wrapped text is where the risk lives, and it is routine: `rules/comments.md` prescribes wrapping code comments at ~160 characters, and several rule and standard docs (`comments.md`, `typescript.md`, `markdown.md`, `biome.md`, `semver.md` among them) hard-wrap their own prose, so a phrase distinctive enough to be worth grepping is long enough to straddle a break. Measured: `grep -n "states it never named"` returned nothing against a Ruby spec whose comment carried that phrase intact across a wrap, and the review finding it was checking was one step from being recorded as already-resolved. **A multiline mode is not the rescue** — the wrap injects indentation and the comment marker, so the bytes are `states it\n  # never named`; bundled ugrep's `-P` matched only once the pattern spelled out `\n\s*#\s*`, and BSD `/usr/bin/grep` has no `-P` at all. Grep the **shortest distinctive token** that cannot wrap — one word, a symbol, an identifier — and before concluding a string is absent, confirm the zero-hit with a second, broader pattern.
- **`>` failing with `file exists`, or `: > file` leaving the content intact, means the tool shell imported `NO_CLOBBER` from the user's interactive rc.** Not a zsh default: the harness snapshots interactive config into tool shells, and whether an rc-set `NO_CLOBBER` survives the replay varies by harness version — measured in both states on the same machine days apart, so never assume either way. Under it, `>` refuses to overwrite an existing file and, because `APPEND_CREATE` stays off, `>>` refuses to create a missing one — and two shapes hide the failure: a backgrounded `nohup … > log 2>&1 &` whose log already exists **never launches** yet the `&` list still exits **0**, leaving the *previous* run's log in place to be read as this run's output (mtime is the tell); and a silently failed `: > file` truncation turns a build-by-append loop into a **doubled** file instead of a rebuilt one (BF-917: a 646-line fix delta became 1183 half-duplicated lines, nearly handed to a verifier as the authoritative diff). The rescue costs one character and is safe under either state: **`>|`** truncates regardless, **`>>|`** appends-and-creates regardless, both identical to `>`/`>>` when clobber is normal — use them by reflex in any tool-call redirect that must overwrite or create. Scope: only redirects written in the `Bash` call itself, including the **outer** redirect of a `nohup zsh -c '…' > log` — script bodies and `zsh -c` children never source `~/.zshrc`. (On this keeper's machine the option came from prezto's `directory` module, not the rc's own `setopt NO_CLOBBER` — `~/.zshrc` now forces `setopt CLOBBER` whenever `$CLAUDECODE` is set, keeping Claude shells clobber-normal deterministically while interactive terminals keep the protection.)

  **In a worktree-isolated session — every `/start wt`, and so every `/auto` fleet session — `>>|` is refused outright**, with the isolation guard's *"this command is too complex to verify that it stays inside the worktree"* message, while plain `>>` and `>|` both run: measured across three calls differing only in the operator, all writing one identical absolute path inside the worktree, so it is the operator and not the path. The reason it is only this one: `>|` is valid in bash and zsh alike, while `>>|` is **zsh-only** and a hard bash syntax error (`bash -n` exits 2 on `unexpected token '|'`; `zsh -n` exits 0), so it does not read as a redirect to a bash-grammar check at all — the same fail-closed family as the heredoc-body tokens in `standards/git.md` § Worktree-isolated sessions. Since the sentence above tells you to reach for `>>|` by reflex, the collision is routine rather than exotic: build the file with a single `>|` write, or, when you genuinely need to append across several commands, `Write` the sequence to `tmp/<name>.sh` and run `zsh tmp/<name>.sh` — script bodies are never analyzed, and `>>|` works normally inside one.
