...
**One mutation is not proof when several wrong implementations reach the same output.** Reverting the exact behavior an assertion names is the first mutation, not the only one: an arm can redden under that revert and still pass under a *different* wrong implementation of the same behavior, because the fixture leaves a coincidental route to the expected result — a sort keyed on the wrong field that still orders this fixture correctly, a filter dropping the right rows for the wrong reason. Where the expected output is reachable that way, name the plausible wrong implementations — the guard removed, the comparison inverted, the right operation on the wrong field — and confirm the arm reddens for each.

Where one slips through, the fixture is what needs fixing rather than the assertion count: vary whatever the implementation is meant to be independent of, so no accidental path reproduces the expected output — and **read the requirements for that word directly**, since a criterion of the form "driven by X, not by Y" names Y as a mutation to run and a fixture value to supply, which the natural fixture omits precisely because Y is irrelevant. Running several mutations also multiplies the chance one silently fails to land — a mutation that did not land leaves the suite green, which under the rule below reads as *the arm pins nothing* and condemns a sound assertion — so `grep` the subject file for the mutated token before trusting the run. Apply each mutation with the Edit tool, which fails loudly on a non-matching `old_string` where a `sed`/`perl` pattern fails silently, and never with `sed -i`, `perl -pi` or an in-place script: an unattended run's permission classifier has refused every such rewrite of a shipped module it was shown (2026-09-22: six of six across four issues), and has refused the Edit as well where the mutation removes a guard or touches a shared component. A refused mutation is an unproven arm, not a pinned one and not grounds for the High below: run the mutant and its arm off-suite against a scratch copy under `tmp/` and say the proof ran off-suite, or report the arm as asserted-but-unproven — never retry a shell form.
