...
| 2026-08-15 retro2 | 2026-08-15 x3 @500k (afternoon, 2h + drain) | 91% w/ >=400k 20% (the 400–474k shoulder; nothing above trigger) | 253 (combined day window) | 2.12 | impl:18/27 tagged | **Kept 500k.** Cadence 6/4/3 per session over ~3h at 5–47 min spacing; three double-compact pairs (mid-heavy-phase compact leaves a 227–281k floor, a consolidating compact follows within minutes → ~150k) — bounded, watch the pair count. $/Mtok out **$696**, best in ledger; ~1.6–1.8h/issue on the reflect-free pipeline (zero per-issue reflect invocations, measured). No quota cutoff at a 1.216M-output 5h peak — tightens the ceiling band from below. |
...
| 2026-08-16 retro | 2026-08-16 x4 @500k (2h + drain) | 94% (>=400k 17% — the pre-compact shoulder; max ctx 458–469k, nothing above trigger) | 159 | 2.29 | origin-tagged 12/12 severity-carrying (the other 20 findings were prose/mechanical — no tag owed); impl:2 | **Kept 500k — verified at n=4.** Cadence 6/5/2/2 per session at 12–47 min spacing; no orbit, no double-compact pairs. $/Mtok out **$586**, best in ledger; $92.85/issue at 7 shipped in 9.9 session-hours, zero waste on every gauge. BF-1147 — the 08-15 orbit casualty — shipped in 2.4h by a fresh session with a fresh floor and 2 compacts, validating the never-resume corollary. New survived floor: 1.11M output in one 5h window at n=4, no stall (measured ceilings 1.21–1.39M). |
| 2026-08-16 retro2 | 2026-08-16 pm x4 @500k (2h + drain) | 84% (>=400k 22% — the shoulder; nothing above trigger) | 132 | 1.83 | origin-tagged 18/29 (plan:7 impl:6 test:5); impl-origin sits on the opus tier (n=3, mean 2.0) — not a tier-routing signal | **Kept 500k — third consecutive clean pass, second at n=4.** Cadence 2/3/1/2 per session over ~3h at 18min–2h10m spacing; no orbit. $/Mtok out **$532**, $70.36/issue at 6 shipped in 11.4 session-hours — both best in ledger, monotone improvement across all four @500k fleets. Churn gauges down alongside (cycles 1.83, C+H/rev 0.33), so the cost fall is not under-review. Non-compaction event: the account's WEEKLY limit hit mid-run at 19:32 UTC (resets Aug 18 2am); sessions resumed after the keeper's manual "limit lifted" prompt and drained clean. Compaction lever now exhausted as the live cost driver — remaining excess routes to the ~108k fixed floor (open follow-up) and the treadmill (fresh share 67%, R 1.17: filing/spec policy, not context). |
| 2026-08-17 retro | 2026-08-17 x3 @500k (12h + drain — first LONG run under the cap) | 91% (>=400k 24% — the shoulder; nothing above trigger) | 176 | 1.54 | origin-tagged 24/65 (test:8 plan:7 impl:5 spec:2 latent:2); impl-origin on the opus tier (n=3, mean 1.7) | **Kept 500k — holds at 12h/n=3.** Cadence 8/8/11 per session over ~16h spans, mostly 34–59 min apart; two bounded minutes-apart clusters (b5a4671e 02:42→02:47→03:01, 66ffb4f7 03:15→03:23→03:38 — the retro2 double-compact shape, recovered to normal spacing after), no orbit. $/Mtok out $614 (long-run mix; still far under the $767–809 uncapped baseline). THE compaction side-effect finding: the cap made sessions ~16–39% hotter per hour (118k out-tok/session-hour at peak vs the 85–102k pre-cap calibration band), so n=3 saturated the 5h output window at 05:58 UTC — 5h07m after launch, exactly the ~102%-of-refill graze the 08-14 ruling accepted — and the measured 5h ceilings moved UP to 1.57–1.74M (vs 1.21–1.39M on 08-14; account likely differs post-weekly-exhaustion, so old ceilings do not transfer). Real loss was recovery, not the cutoff: 25.2 session-hours dead AFTER the 1:30am reset (worst 8.4h) because auto-stall-watch has never matched the live `claude agents --json` schema (no `id` field, no `background` kind — 371 ticks, 0 flags ever); keeper attach at 09:54 CDT was the actual recovery. Overnight/unattended sizing corollary: n=2 × 12h (~237k/h burn vs ~348k/h refill) never saturates and out-produces a stalled n=3; n=3 remains right only when an operator is awake to recover. |
| 2026-09-24 retro | 2026-09-24 x3 @500k on opus[1m] (28h budget, operator-stopped at 6.4h; epic BF-1826) | 96% (>=400k 36%; five pre-compact reads above the trigger at 512–557k) | 239 | 2.15 | impl:51 of 143 tagged (test:54 largest); impl-origin on the opus tier (n=10, mean 4.4) | **Kept 500k; a new orbit mechanism, carry-over rather than threshold.** 798b933f orbited twice: 4 compacts in 4 min from 18:07 UTC, and 3 in 3 min from 21:35 UTC, both flagged by the harness's own thrash detector. The second was terminal, and the operator killed the session at 21:52. The orbiting compacts carried forward `postTokens` 143–154k against 33–40k on normal ones, so the first call after compaction sat at ~330k rather than ~165–175k, leaving a ~130k band under the trigger. The carry-over is path-scoped rules, not the preserved-message chain (13–17k chars for both kinds): every compact that carried 142k or more had the project's `api.md` rule (365KB, scoped to `apps/api/**/*.rb`) plus `environments.md` injected as `nested_memory` after the last call and carried through (2 other compacts preceded by the same injection stayed low), and the first tool call after compaction re-injected them, whatever it touched, because the compaction's restored-file references named `apps/api` Ruby files, refilling the context to ~440k in a single turn. The harness's "file too large" hint was right about the size and wrong about the file: the burst's tool results were ~4k chars each. A threshold change cannot fix this; shrinking the injected rule can. The other sessions had isolated high-carry compacts (7301912d 143k, ee9d64f3 95k), never back to back. A fresh targeted `/auto BF-2071` recovered the issue with 0 compacts, so never-resume held again. $/Mtok out $875. |
