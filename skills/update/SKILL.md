...
- **`~/.claude` is on a branch other than `main`, or the user just wants it back on a clean `main`** — the pull is
  ...
  `/update` after: `bash -c "$(curl -fsSL https://raw.githubusercontent.com/alienfast/claude/main/scripts/sync-main.sh)"`
  (that form, not `curl | bash` — a child inside update.sh reads stdin and would swallow the piped script).
