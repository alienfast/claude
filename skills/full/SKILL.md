...
6. **Gate 2 — deferred-items filing inside `/quality-review`** (user input, sub-step 6 — fires only when there are non-trivial items to file as issues; trivial `fix-now` items auto-apply with no prompt)
