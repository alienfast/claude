...
- **Background-job `/full wt`.** Launched as a background job rather than the usual foreground agent-view session, the implementation delegations (`/start` Step 8) come under the `worktree.bgIsolation` guard — see `/start` Step 8's "Background-job runs" note for the `EnterWorktree(path=…)` / `worktree.bgIsolation` levers. Foreground runs need nothing.
