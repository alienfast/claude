...
- **Background-job `/full wt`.** No special handling. `/start` Step 0 enters the worktree with `EnterWorktree(path=…)`, which registers isolation so the implementation delegations' (`/start` Step 8) `developer` writes land in the worktree with the guard **on** in both foreground and background — see `/start` Step 8's "Background-job runs" note. Never disable the isolation guard (`worktree.bgIsolation: "none"`), never edit settings to weaken it, and never let the orchestrator implement the code itself; a still-blocked delegate write is a STOP-and-surface, not a lever.
