...
- **The Step 3 bar-check is not skippable, and a skipped check is not a disclaimer.** A batch whose candidates did not pass the bar-check is surfaced only, never filed — a "verification disabled" note in the body does not license the filing. The first contributor proposal PR (alienfast/claude#6) carried three consecutive batches filed that way; adjudication re-verified every claim and found the defect rate the guard exists to catch.
...
8. Prescribes against observed behavior, not general truth — the candidate names what an unprompted agent actually did (or demonstrably would do) wrong that this text prevents. Well-known doctrine agents already follow is over-prescription: it dilutes the load-bearing rules and trains readers to skim. A measured tell wrapped in textbook framing compresses to the tell plus the measurement; pure doctrine with no tell is a drop.
9. Exceptions update the sentences they falsify — when the draft adds an exception or carve-out to an existing rule, grep the config for sentences citing that rule as absolute ("bans X altogether", "never Y"); every one the exception falsifies must be updated in the same proposal, or the config ships contradicting itself.
...
2. **Build the body** into a unique tmp file — `mkdir -p tmp`, then `body_file=$(mktemp -u tmp/reflect-improvement-XXXXXX)` (`-u` — name only, no file created: the Write tool refuses a pre-existing file it has not Read, so plain `mktemp` breaks the next step. No suffix — BSD `mktemp` only substitutes a template that ends in the `X`s). Shape:
   ...
   **Write the proposed config text self-contained** — mechanism, date, magnitude — with no team-local vocabulary: no team-prefixed issue IDs and no team-local label or state names inside the diff's prose (test fixtures replaying real history exempt). The issue body and the eventual commit message are the provenance home for IDs; the keeper strips them at adjudication otherwise (skills/keeper Step 2), so filing them clean saves the batch a rewrite round.
...
5. **Wire the filed issue's collision edges** — [standards/issue-spec.md](../../standards/issue-spec.md) § Certification includes collision edges: filing hands the pool a pickable issue, so sequencing against open siblings must already be mechanical. `<new-ID>` below stands for the identifier item 3 captured as `new_id` — that capture ran in item 3's own Bash call, and shell state does not survive across separate Bash invocations, so a literal `"$new_id"` here sends an empty identifier to the API; substitute the resolved identifier.
   ...
   - the hit is a prerequisite of this filing or both edit the same region of a method body (not merely the same file, nor disjoint branches of one long method — standards/issue-spec.md § Certification includes collision edges), the hit is itself `specified`, and this filing is a non-keeper (`specified`) filing → `linear-cli relations add <existing-open-ID> <new-ID> -r blocks` (existing blocks new — the no-discernible-order default of standards/issue-spec.md's direction rule — because `next-candidates.sh` hides blocked issues, which is what actually prevents concurrent pickup);
   - same file or same method in different regions, or a prerequisite/same-method hit that is uncertified, `needs decision`, or `solo` (a blocker that will never ship unattended), or this filing is a `--keeper` batch → `-r related` plus a comment naming the file — a `related` partner in flight soft-de-ranks this issue, while wiring `blocks` behind a blocker that never ships unattended would strand it invisibly;
...
**A stale comment or docblock in source code is an `apply-now` in-place correction — fix it, never file it.** A sweep reads application code to test config claims (Step 2), so it routinely diagnoses comments asserting behavior the code no longer has. `~/.claude/rules/comments.md` § Scope puts these in scope and forbids deferring them: the "file the change does not otherwise touch" exception licenses *leaving* a comment alone, never converting it into an issue. Filing one spends a pick, a worktree, and a review cycle to change prose this run already has open, and the deferred issue reads as real work to whoever picks it up. Fix them in the working tree with the other in-place corrections and list them in the Config Audit report; a wrong comment is a defect, not a finding awaiting triage. Escalate to `propose` only when the fix needs a behavior decision the sweep cannot make (the comment may be right and the *code* wrong).
...
- Filed: <PL-XX (Backlog, specified — or "Backlog, keeper — uncertified" for a --keeper batch) — append `(label not attached)` on exit 2, and/or `(collision edge to BF-YY not wired — add manually)` on an item 5 miss; or none — reason if skipped>
