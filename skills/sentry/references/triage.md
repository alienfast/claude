...
**`merge` is unavailable — same-cause fingerprints are joined in Linear only.** `sentry issue merge --help` states it: "Only error-type issues can be merged (the API rejects performance/info issues)." The API's reported response body is `400 ["Only error issues can be merged."]`; it has not been verified against a live project, because `merge` is this skill's one irreversible mutation and carries no `--dry-run`. So a **Duplicate** disposition on a performance issue means one owning Linear issue naming every Sentry short-id in its description, with each of them resolved individually at fix time — never a Sentry-side merge. **Neither an External Link nor a note can reach one.** `scripts/link-linear.sh` refuses a `performance_*` issue with exit 3 because Sentry's `external-issue-actions` endpoint answers `Could not find the corresponding issue for the given groupId` for it, and its `external-issues` and `comments` listings both answer `{"detail": "You do not have permission to perform this action."}` — at exit **0**, since `sentry api` passes the body through (measured 2026-09-16 on four N+1 issues, where the same link landed on six error issues in the same minute). The Linear description is therefore the only place a performance issue's mapping exists; the description convention below is load-bearing for exactly this case.
...
Before dispositioning, search Linear for each candidate's short-id — the filing convention below guarantees it appears in the Linear issue description. Follow the `/linear` skill for the commands. For an error issue, `sentry api "issues/<numeric-id>/external-issues/"` is the second source — it lists the External Links the skill (or a human, via "+ Link issue") attached, each with `displayName` `BF#1939` and the Linear `webUrl` — and it catches a mapping made by hand in the UI that never reached a description. It answers a 403 body for a performance issue, so the description search stays the one that covers both queues. An issue with a Linear match is **filed**: show the Linear identifier and state in the queue table; only re-open it (comment, never re-file) when Sentry shows a regression after the Linear issue shipped.
...
- Bidirectional link: the Linear description carries the Sentry permalink + short-id (above); after filing, attach the Linear issue as an **External Link** on the Sentry issue with `scripts/link-linear.sh <org/project> <SHORT-ID> <TEAM>-XXXX` — the sidebar's "+ Link issue", which Linear mirrors as a `sentry` attachment on its issue (title + permalink) the moment the link lands (measured 2026-09-16). Not a comment: a note is Activity, invisible in External Links and to Linear.
...
- Nothing is confirmed — reads and writes alike. The writes are `resolve`, `unresolve`, `archive`, `merge`, and the external-issue link (`scripts/link-linear.sh`, an `external-issue-actions` POST; its inverse is `DELETE sentry-app-installations/<uuid>/external-issues/<id>/`, documented but not yet exercised here).
