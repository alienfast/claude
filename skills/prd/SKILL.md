...
5. **Certify**
   ...
   One class of sub-issue certifies **gated**: work only a human can perform — customer outreach, vendor contact, production console operations, a confirmation pass the body itself assigns to a person. The quality bar tests spec shape, not agent capability, so these pass it and then sit rankable in the certified pool until a fleet session picks one and stalls. Attach `needs decision` alongside `specified` here, with a comment naming the human action required — the spec is right, the work is gated ([standards/issue-spec.md](../../standards/issue-spec.md)); `/auto-prep` Step 2 applies the identical disposition later, but only after the issue has already spent time rankable.
