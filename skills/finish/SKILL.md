...
- **2 (worktree needs work before the merge can land — act in the worktree)** — two shapes, distinguished by the script's stderr tag.
  ...
  `GENERATED-DISCARD:` — a `merge=ours` driver on generated file(s) kept the worktree branch's copy and silently dropped `<SOURCE_BRANCH>`'s hunks. The merge is **already committed** — there are no conflict markers and no `MERGE_HEAD`, so do NOT `merge --abort`. Regenerate the listed artifacts in the worktree from the merged sources (the project's documented schema/codegen regeneration), then `git -C '<WT_DIR>' add <files> && git -C '<WT_DIR>' commit -m 'Regenerate merged artifacts'` and re-invoke the same Step 2 line. **Auto mode:** if the project documents no regeneration command, do not guess — stop with `BLOCKED-ON-REVIEW: <ISSUE-ID> — merge=ours driver dropped <SOURCE_BRANCH>'s generated hunks (<files>); regeneration needs a human. Worktree preserved.` (A genuinely-intended discard re-runs with `_FM_SKIP_OURS_GUARD=1` — interactive only, never in auto mode.)
