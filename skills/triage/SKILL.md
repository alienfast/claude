...
It paginates the raw API (`linear-cli issues list` returns 50 rows by default, silently — linear skill gotcha #25; `--all` exists but the raw hatch is what carries relations and comments — and a `first:250` cap truncates a 500-issue Backlog just as silently), fetching every issue whose state *type* is triage, backlog, or unstarted, with description, comments, relations, and creator. For each issue it computes:
...
**Resumable and safe to re-run.** An issue with a pending proposal is skipped (`--force` re-scans), and so is one under `applied/` whose pool row's `marker_sha` differs from that proposal's `sha` — the pool file predates the apply. A matching sha means the cheap pass saw the marker and still flagged the issue, so it is re-scanned: `applied/` is never cleared, and skipping on presence alone would silently block every later run's re-scans. `unchanged` and `claimed` rows are never scanned; a failed group leaves no proposals, so the next invocation picks its issues up. `--max-groups 1 --ids A,B` is the trial run. Progress and per-group cost go to `tmp/triage-scan.log`, with group ids prefixed by the run's start time so two lanes' rows do not collide; `triage-scan.sh --summary` prints the counts by disposition and the non-keep lists with evidence. Two lanes may run concurrently at a lower `--concurrency` each. **A refused call stops the run** (exit 3): the account's session limit answers `is_error: true` under `subtype: success`, with `.result` naming the reset time. Two shapes, measured on the first full scan (2026-09-22, 28 refusals): a group **in flight** when the limit lands is cut off mid-run — `num_turns` 22–43, no `structured_output`, and `total_cost_usd` for the work it had already done (six sonnet groups, $13 between them) — while a group **launched after** the limit is refused at `num_turns: 1`, zero tokens, $0, in 3–4 s (the other 22). Stopping at the first refusal therefore recovers nothing already spent; it ends a pointless launch loop and surfaces the reset time. Re-run both lanes after the reset: everything written before it stands, and the cut-off groups are re-scanned because they left no proposals.
