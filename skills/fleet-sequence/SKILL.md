...
Run from the project the issues belong to, with the main checkout **clean** and on the branch the PR should target (usually the default branch). The script refuses a dirty tree, a detached HEAD, a live fleet, a running sequence, and — in `pr` mode — a launch branch missing from origin or a stray `seq/<first-id>` branch no marker accounts for:
...
Surface the script's output verbatim: the sequence line, the runner pid and log path, and its note that the main checkout is never moved. **Do not wait in this session** — the runner is detached (`nohup`, reparented to launchd) and lives for hours; there is nothing to poll here, and `no-blind-sleep.sh` would refuse the attempt anyway. Close by telling the user how to watch, and that the main checkout stays theirs — other worktree sessions (a targeted `/auto`, `/start wt`) may run alongside, and only a dirty tree gets in the way (each child's `/auto` halts on it):

- `/fleet-sequence status` — mode, branch and base, the PR once it exists, any per-issue fork key currently set, per-issue session id, registry liveness, outcome and the commit it landed at, the branch's position against its base, the runner's own liveness, and the log tail.
...
When the last issue has landed (`pr` mode): `integration-pr.sh` pushes the branch, finds or opens the PR onto the launch branch with the roster body, and reports how many commits the base gained during the run — that many is one catch-up merge (GitHub's *Update branch*) before it merges. Then one closing child, `claude --bg -n "fleet-sequence pr-update" "/pr-update"`, runs from a throwaway worktree on the branch (`tmp/fleet-sequence-pr-update`, removed when the session ends; left in place with a WARN if it times out), waited on up to `FLEET_SEQUENCE_PR_UPDATE_TIMEOUT` (30 min; `FLEET_SEQUENCE_PR_UPDATE=0` skips it). A PR that cannot be opened fails the run with the ships kept — re-running the same list repeats only the PR step.
...
- **The same release shape as an epic fleet, steered differently.** Both land merges ref-only on an integration branch and end in one PR from the same script, `integration-pr.sh`. An epic fleet parks the whole checkout (detached HEAD plus the checkout-wide `start.wt-source-branch`) because its sessions pick their own issues; a sequence knows each issue before its dispatch, so it steers with per-issue keys and leaves the checkout alone. The two still refuse each other — a sequence never starts mid-fleet, and `fleet-launch.sh` refuses while a sequence's runner is alive — because `solo` work is solo, not because they share state.
