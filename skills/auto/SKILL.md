...
**Re-anchor cwd to the main checkout first.** A prior iteration may have ended with cwd inside `.claude/worktrees/<id>` (`/start wt` cd's in; only the merge path cd's back). The main checkout is the first `worktree <path>` line of `git worktree list --porcelain` (unambiguous even with spaces in the path); `cd` there before anything else. **Every read or write of `tmp/auto-state.json` — Step 0, Steps 1–2's terminal transitions, Step 4, and all Error Handling transitions — uses `<main-checkout>/tmp/auto-state.json`** (carry the resolved absolute path through the iteration); a state file touched inside a worktree would fragment the run state (resetting the breaker and skip lists, making halts non-sticky) and be deleted with the worktree.

**Background-job note (bgIsolation).** In a background session the harness's worktree-isolation guard can refuse Write-tool edits outside the registered worktree — including `<main-checkout>/tmp/auto-state.json`, which this skill REQUIRES to live in the main checkout. That file is gitignored, run-scoped bookkeeping owned by this skill, not project code: when the Write tool is blocked *by this guard* (not by the read-before-Write requirement — that one you fix by Reading first), write it via the shell (e.g. `printf '%s\n' '<json>' > <main-checkout>/tmp/auto-state.json`) instead of relocating it into a worktree (fragments the run state) or setting `worktree.bgIsolation: none` (removes the guard for all code edits, not just this file).
