...
Model/permission defaults follow `/auto`'s unattended-run prerequisites (`--model 'opus[1m]' --effort xhigh --autocompact 150000 --permission-mode auto` — `auto`, not `acceptEdits`, because acceptEdits only auto-accepts file edits and the first gated Bash command would stall a background session at an unanswerable prompt; `--autocompact 150000` because on `opus[1m]` the default compaction threshold sits near the 1M window, so a `/loop /auto` session accumulating across iterations never compacts — the 2026-08-13/14 fleets processed 91% of their billable volume at >200k context, and cache reads scale linearly with context size, which is what let 3 sessions exhaust a 5h window that 9 leaner sessions survived. `/auto`'s cross-iteration state lives in `tmp/` handoff files and Linear, not in context, so compaction between issues is loss-free; whether 150k is *too* aggressive is measured, not assumed — watch `fleet-metrics.py`'s context-size distribution fall and its review-churn gauges hold steady across the next runs, and raise the threshold if churn climbs while context cost drops); anything after `--` passes through to `claude --bg` and overrides them. `FLEET_PROMPT` overrides the dispatched prompt — e.g. `FLEET_PROMPT='/loop /auto BF'` to team-scope the run.
