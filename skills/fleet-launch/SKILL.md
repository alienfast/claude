...
Model/permission defaults follow `/auto`'s unattended-run prerequisites (`--model 'opus[1m]' --effort xhigh --autocompact 300000 --permission-mode auto` — `auto`, not `acceptEdits`, because acceptEdits only auto-accepts file edits and the first gated Bash command would stall a background session at an unanswerable prompt; `--autocompact 300000` because on `opus[1m]` the default compaction threshold sits near the 1M window, so a `/loop /auto` session accumulating across iterations never compacts — the 2026-08-13/14 fleets processed 91% of their billable volume at >200k context, and cache reads scale linearly with context size, which is what let 3 sessions exhaust a 5h window that 9 leaner sessions survived. `/auto`'s cross-iteration state lives in `tmp/` handoff files and Linear, not in context, so compaction between issues is loss-free. The value cannot go much lower: compaction triggers at ~90% of the window and resets to a ~115–121k floor of re-injected fixed overhead (system prompt + CLAUDE.mds + rules + skill bodies), and real fleet workloads ingest up to ~130k in a single call (skill-chain loads, big Reads), so the trigger must clear floor + one worst-case ingestion — 150000 left a <20k band and thrash-aborted all three 2026-08-14 sessions before their first pick, and 250k still quick-refilled in replays of six real session workloads; 300k was clean in all six (doc/compacting-investigation.md, verdict log). Watch `fleet-metrics.py`'s context-size distribution and review-churn gauges before moving it in either direction); anything after `--` passes through to `claude --bg` and overrides them. `FLEET_PROMPT` overrides the dispatched prompt — e.g. `FLEET_PROMPT='/loop /auto BF'` to team-scope the run.
