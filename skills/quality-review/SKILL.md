...
**The sibling sweep belongs to item 2's `developer` delegations alone.** Wherever this block mentions it, that text is scoped to the Critical/High/Medium fix delegations item 2 dispatches: every other `developer` fix site in this skill (the `developer` entries in the worktree-isolation blockquote above Step 2) composes its own prompt from this block's shape and drops every sweep-dependent line it finds — those lanes are bounded by their own gates, and *this* sweep escaping into them would be bounded by nothing, because its bound is the `Findings` file list only item 2's delegations carry. The prose lane's class-completion rule (item 5) is not that escape but a sibling rule in the same family, carrying its own bound — the prose-only definition — in place of the file list.
...
**4. Re-review (MANDATORY)** — fixes are not complete until re-reviewed. Spawn `quality-verifier` (per Dispatch tiers) **scoped to verification of the fix delta**, not a fresh review of the whole change:
...
- **Prose-only** (Step 6 sub-step 2's prose-only rule, its three exclusions included — the fix edits only comments, doc-strings, or standalone doc/rule text; the severity label the reviewer gave it does not matter): the orchestrator applies the fix **directly** — no dispatch, per Dispatch tiers — as its own isolated edit confirmed by its own `pnpm check`, before this cycle's substantive fixes land. The isolation is what makes a failing check attributable — item 3's check covers the whole batch — and a failure means the edit was not actually prose-only, so treat it as a regression at its real severity rather than as a landed prose fix. A class-shaped prose defect — the same stale claim, wrong cross-reference, or contradiction duplicated across sibling files — is completed across every sibling in the same pass, per the **prose class completion rule below**. A prose finding whose fix already survived one attempt this run gets the **ping-pong rule below**, not another minimal patch. It is **never loop fuel**: it neither makes a re-review mandatory on its own nor counts against convergence, and it adds nothing to the `Cycles: N` value in the Output block (the re-review dispatch that surfaced it still counts — a cycle whose findings happened to be all prose-only is still a cycle). Whichever phase drains it — this cycle's fixing phase when the loop continues, the convergence drain when this re-review is the converging one (see Termination below), or the soft ceiling's lane drain — the handling above is the same.
- **Mechanical** (definition below): the orchestrator fixes it now, directly, and verifies it with a **targeted fix confirmation** — not a review cycle.
...
- Spawn a single `quality-verifier` re-review scoped to **only** the files touched by the deferred fixes:
...
- If the re-review surfaces new **Critical/High/Medium** findings, first split them by **provenance** — per finding, not per batch:
  - **Regression** — the finding implicates the deferred-fix delta: it describes behavior the fix-now applications (direct or dispatched) changed or broke. Test: the finding would have been FALSE against the code as it stood when Step 4 assigned its verdict, and became true only because of the deferred fixes.
...
- If any **regression** findings remain after the split, make exactly **one** corrective pass: delegate to `developer` (sonnet-tier, per Dispatch tiers) to fix the regressions, then re-run `pnpm check`. Do **not** re-enter the Step 5 loop.
...
#    "<finding>\n\nLocation: <file:line>\n\nRationale: <rationale>"
#    Severity-carrying items (routed Critical/High/Medium — pre-existing gaps and
#    narrowed-fuel Mediums of the change alike) instead use the
