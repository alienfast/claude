...
- If the **mandatory** re-review's split leaves any **regression** findings, make exactly **one** corrective pass: delegate to `developer` (sonnet-tier, per Dispatch tiers) to fix the regressions, then re-run `pnpm check`. Do **not** re-enter the Step 5 loop.
  ...
  - **Open items:** the surviving Critical/High/Medium regressions, **plus** the implicated deferred-item fixes per the rule above, **plus** every not-yet-offered *fileable* item — sub-step 6's entry gate defines that set, and the whole set lands here regardless of which path routed each item into it (the user never reached sub-step 6, so fileable items are not silently dropped — they are surfaced for manual follow-up). `note-only` items are **not** swept here — see `Deferred dropped` below.
...
#    defer-as-issue items and the Nice-to-Have findings sub-step 5's re-reviews routed alike:
#    "<finding>\n\nLocation: <file:line>\n\nRationale: <rationale>"
...
**A parent at Linear's sub-issue nesting cap is not a filing failure — do not stop, do not downgrade.** When the helper exits non-zero but its stderr names the created issue (`created BF-XX but 'relations parent …' failed`), re-read the issue before applying the create-failed rule below: if it exists, Linear **rejected the parent edge** rather than the create. A direct retry then returns `invalid parent`, deterministically — the parent is already at the nesting cap, so every item in the batch fails identically and retrying never helps (BF-687 sat ten levels deep; all four of its filings hit this). Treating it as unfiled would stop the batch and downgrade a passing verdict over a benign, fully-recoverable condition, and this shape grows by construction: a review of issue N files sub-issues of N, so each generation sits one level deeper. Instead: wire the linkage as a peer edge — `linear-cli relations add <new-ID> <ISSUE-ID> -r related` — continue filing the remaining items, skip the independent parent re-query for these (it will correctly print no parent, and the `(sub-issues of <PARENT>)` suffix does not apply to them), and annotate each in the verdict block as `BF-XX (related, not sub-issue — parent at Linear's nesting cap)`. **Also verify the labels attached**: the helper links the parent *before* applying labels, so this path exits with the issue unlabeled — and without `specified` it is invisible to `/auto` and unranked by `/next`. Query each issue and attach any missing label with `~/.claude/scripts/linear-add-label.sh <ID> <label>`, **one call per label** (the script takes exactly two arguments, and zsh does not word-split an unquoted `$var` holding several names — see `~/.claude/CLAUDE.md`).

**Filing failure — two distinct modes.** They differ in what is known to exist, so they carry different remediations and different verdict effects; under both, every chosen item still lands in exactly one verdict field.

- **Create failed** — `create_status` is non-zero and not 2, **and the nesting-cap case above does not apply** (the stderr does not name a created issue, or a re-read shows no such issue). The helper could not create, link, or verify the issue (its one-line stderr says which), so from this seat the issue may not exist in Linear at all: treat the item as **unfiled**, and **stop filing immediately — never attempt the remaining items** (their creates would run against the same failing API, widening the set whose existence is uncertain). This failure changes the verdict: **downgrade the run to `terminated-with-open-items`** — the same transition sub-step 5's regression cap uses — because the affected items have nowhere honest to go under a passing verdict: they are not filed, the reply selected them so they are not dropped, and `Open items` is read by `/finish` and `/start` only on a non-passing verdict, so routing them there without the downgrade would bury them. This holds in auto mode too: the failing verdict makes `/finish auto` abort and `/auto` surface the issue to a human, the intended path for a run whose filing could not complete. Populate the verdict block:
