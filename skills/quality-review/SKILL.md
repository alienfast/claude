...
**Entry gate — skip sub-step 6 entirely if no *fileable* items remain.** Fileable items are the `defer-as-issue` items, any **severity-carrying items** routed here (pre-existing Critical/High/Medium findings — from Step 5 item 5's fix-loop pre-existing lane, from Step 5's auto-mode ceiling resolution, or from sub-step 5's re-reviews), plus any new Nice-to-Have findings appended by sub-step 5's re-review (prose-only findings are never appended and never fileable — see sub-step 2 and sub-step 5). Auto-applied `fix-now` items are already fixed and are not fileable. If none of those remain there is nothing to file — skip the prompt and emit the Output-section verdict block (the schema under `## Output`, not the sub-step 6 template below). Skipping sub-step 6 does **not** exempt the auto-applied items from the verdict: they MUST still be listed in that block's `Deferred fixed in-session:` field, and any `note-only` items — from sub-step 2 or classified by the sub-step 5 re-review — MUST still be listed under `Deferred dropped:`. (This is the common case when every deferred item was `fix-now` or `note-only`: nothing is fileable and the run reaches Output with no prompt at all.)

Render the remaining unfixed items using the template below, then ask the question that follows. The render is REQUIRED regardless of prompt mechanism (markdown body, AskUserQuestion description, etc.) — do not collapse to a single sentence; assume the user is context-switching across parallel sessions and cannot scroll back to sub-step 3. Preserve original sub-step 3 numbering: auto-applied `fix-now` items are shown under "Auto-fixed in-session (for context)" with their original numbers (e.g., 1, 2), and the fileable `defer-as-issue` items keep theirs (e.g., 3, 4), with any new sub-step 5 re-review findings appended after. **Severity-carrying items (routed by Step 5 item 5's pre-existing lane, by Step 5's auto-mode ceiling, or by sub-step 5) render INSIDE the "Suggested defer as issue" group** — severity in the tag slot (e.g. `[MEDIUM]`), continuing the numbering (if sub-step 3 numbered no items, number from 1; severity-carrying items precede sub-step 5's appended Nice-to-Have findings) — because membership in that group is what makes the `suggested` reply (and auto mode's default) file them. Omit a sub-group header entirely if empty; do not print "(none)".
...
**Auto mode:** do not ask — still render the grouped list (it is the audit trail), then proceed as if the user replied `suggested`. Filing the defer-as-issue group — including any severity-carrying items routed into it — keeps discovered work flowing back into Linear for later `/auto` iterations without over-filing. If the state-name fallback below would need to ask the user, file into the team's default state instead and note the deviation in the verdict block — in an unattended run a re-statable issue beats a dropped finding.
...
```bash
mkdir -p tmp
# 1. Write description to a unique tmp file. Body shape for ordinary defer-as-issue items:
#    "<finding>\n\nLocation: <file:line>\n\nRationale: <rationale>"
#    Severity-carrying items (pre-existing C/H/M routed by Step 5's lanes or by sub-step 5) instead use the
...
#    (or omit it) for ordinary defer-as-issue items — they cleared gate 3 precisely
