...
- **Mode probe**: `git -C ~/.claude config --get reflect.keeper`. Prints `true` → keeper mode. Anything else → contributor mode. Never instruct a non-keeper to set the flag — it is the keeper's one-time setup, not a preference.
