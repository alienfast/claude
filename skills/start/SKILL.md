...
**Argument parsing.** Tokens are case-insensitive (`wt`, `WT`, `Wt`) and position-agnostic. Parse in this order: **(1)** strip recognized tokens — `wt` and `__from-full` (see the next paragraph); **(2)** verify exactly one non-token argument remains, otherwise error; **(3)** pass that remaining argument through `~/.claude/scripts/detect-issue-id.sh --validate-only --input <arg>` to normalize and validate (the script enforces `^[A-Z]+-[0-9]+$` and uppercases). The strip-then-validate ordering matters: passing `__from-full` to `detect-issue-id.sh` would produce a confusing regex-failure error. Multiple IDs or duplicated `wt` tokens are errors.

**Private flag — `__from-full`.** When present, this token signals that `/start` was dispatched by `/full` (not the user). Strip it before validation (per the parsing order above), set the internal flag `FROM_FULL=1`, and continue. It is case-insensitive and position-agnostic, compatible with both `wt` and non-`wt` modes, and does NOT count toward the "exactly one non-token argument" check. It affects only Step 10's tagged-line text (see Step 10 sub-item 6) — every other step is unchanged. The flag is meant to be private to the `/full ↔ /start` coupling; if a user types it directly, accept silently. (We cannot detect the caller, and erroring would break `/full`'s legitimate use. The recovery hint embedded in the Step 10 state-signal variant — `(Manual recovery if session died: /finish <ISSUE-ID>[ merge].)` — ensures a directly-invoking user is not left without a next-step.)
...
6. **Next steps (tagged final line — see `standards/lifecycle-tags.md`)**: Emit ONE line, structured as `<TAG>: <ISSUE-ID> — <one-line summary including the recommended next command>`. Tag is mechanical, keyed off the verdict from Step 9:
   - `passed-clean` / `passed-after-fixes` → branch on the Step 0 `FROM_FULL` flag (case-insensitive `__from-full` token):
     - `FROM_FULL` **unset** (user invoked `/start` directly) → `READY-FOR-FINISH: <ISSUE-ID> — <impl summary>. Run /finish <ISSUE-ID>[ merge]` (append ` merge` when in a `/start wt` worktree).
     - `FROM_FULL=1` (dispatched by `/full`) → `READY-FOR-FINISH: <ISSUE-ID> — <impl summary>. /full continues to Step 2. (Manual recovery if session died: /finish <ISSUE-ID>[ merge].)` This variant reads as orchestration state, not a user-facing next-command. `READY-FOR-FINISH:` remains the keyed token in both variants, so `/full`'s Step 2 parser (which keys off the first token) handles both transparently. The trailing parenthetical is a safety net for two cases: (a) the session is killed between `/start` returning and `/full` Step 2 dispatching, so the user needs a recovery command (see `full/SKILL.md` "Session interruption" paragraph); (b) a user accidentally typed `__from-full` directly to `/start` outside of `/full`, in which case the parenthetical is the only next-step they get. Rationale for the state-signal phrasing: the unconditional `Run /finish ...` text trips the orchestrator's "skill's final line = end-of-turn summary" heuristic and causes `/full` to skip Step 2; the conditional `(Manual recovery if session died: ...)` reads as a fallback note, not an unconditional imperative. (See `full/SKILL.md` Step 1 action 4 and the "TodoWrite priming fails to hold" edge case for the failure history.)
