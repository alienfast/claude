...
> **Background-job runs — the guard is already handled; never break it.** Step 0 entered the worktree with `EnterWorktree(path=<WT_ABS>)`, which registered it as the session's isolation root. `developer` subagents inherit that registration, so their writes land in the worktree with the isolation guard **on** — in both foreground and background sessions, with nothing extra to do here. If a delegate's Write is nonetheless blocked by the guard, that is a **hard stop, not a license to weaken isolation**: do **not** set `worktree.bgIsolation: "none"`, do **not** edit `settings.json` / `settings.local.json` to change isolation (the auto-mode self-modification classifier is right to block that), and — above all — do **not** take over the implementation yourself. The orchestrator-only rule at the top of this step is absolute and is **never** suspended by a delegation-mechanics failure. Re-confirm Step 0's `EnterWorktree` registration succeeded (cwd is the worktree; it was entered from the main checkout, not via a shell `cd`); if writes are still blocked, STOP and surface the block to the user rather than improvise.
...
**Resumption.** `/start` is idempotent on the same issue: re-running `/start PL-13` after a `/checkpoint`-and-stop should detect the issue's existing `In Progress` state and the existing branch, skip Step 0's worktree *creation* (sub-step 1's setup script) — but still re-enter the existing worktree via `EnterWorktree(path=<WT_ABS>)` so the resumed session's isolation is registered, else a background resumption's `developer` writes hit the guard. Sub-step 2's precondition applies: **re-anchor cwd to the main checkout first** (a manual resumption may launch with cwd already inside the worktree, which would trip the same-cwd refusal), and pass the **absolute** path `git worktree list` reports for the issue-keyed `.claude/worktrees/<id>` (the tool rejects a relative path as not a registered worktree). Then pass *through* Step 3 — which recognizes the issue is already claimed by me and short-circuits, skipping only the `linear-cli ... update` call per its idempotent branch (do not skip Step 3 wholesale; its availability check still runs) — then resume at the implementation phase. If Step 9 (review) had previously run, the existing `tmp/quality-review-verdict-<issue-id-lowercased>.md` file (e.g., `tmp/quality-review-verdict-pl-13.md`) is still consulted by `/finish` Step 1.5 — the user can decide to re-run `/quality-review` to refresh it, or skip ahead to `/finish` if the prior verdict still applies.
