...
3. **Run the verification script — Steps 2–5's actions in one call, with Step 6's exploration dispatched in the same message** (except with the `interactive` token, which dispatches no exploration — see the exceptions below). The digest (sub-step 2) and `STATE=`/`ASSIGNEE=` (sub-step 1) are already in hand, so make the decisions FIRST, before composing the call:
   ...
   **Do not merge the streams — the verdict is the first line of *stdout*, but the LAST line of a `2>&1` capture.** The script sends everything else to stderr *including* `pnpm check`'s own stdout (`pnpm check >&2`), so stdout holds exactly one line and the contract above is exact — but merged, hundreds of check lines precede it, and the capture routinely exceeds the harness's inline output limit, leaving the tool result showing only a head preview starting `== cwd confirm ==` with no verdict in it at all. Redirect **stderr only**, which keeps the verdict inline and small while still persisting the diagnostics this step needs (the `FAILED-CLAIM` 5xx probe, `WARN:`, `SOURCE_BRANCH=`):
   ...
   - `VERIFIED` → Steps 1–5 complete, the Working Application Contract is in effect; proceed to Step 6 (whose exploration is already running) — **unless the `interactive` token was given, in which case sub-step 4 halts the skill here instead of continuing to Step 6.** **One part of Step 5 is NOT discharged by this verdict:** `start-wt-verify.sh` runs `pnpm check` and no other suite, so when this issue's changes will touch a suite `pnpm check` does not run (in basefund, rspec — see its `CLAUDE.md` § Running RSpec), the Working Application Contract's rule 1 baseline for that suite is still owed. Capture it — and record its example count — now, before implementation: `wt` mode never reaches Step 5's body, where that instruction is stated.
...
1. **Verify placement — run after EVERY delegation, gated on `IS_WT` alone, never on agent type.** Do not skip this for `quality-reviewer`/`architect` delegations just because their prompt carries no WRITE-PLACEMENT block — that scoping (the "Delegation format" intro above, "split the isolation instructions by what the delegation actually does") is a decision about what goes in the *prompt*, not about what the orchestrator itself verifies. Every agent type is provisioned with Write/Edit tools; any of them — including a reviewer that "helpfully" applies a one-line fix under a mis-bound registration — can write into the main checkout, and this check costs one `git status`-equivalent pass regardless of what the delegate reports or doesn't report. Gate purely on `IS_WT` (defined in Step 0; also armed by Step 5's short-circuit, which as of the fix above always pairs it with a baseline). If `IS_WT` is false, skip this item entirely: there is no separate main checkout, and running this check anyway would misread the delegate's own legitimate diff as contamination (a false `BLOCKED-ON-REVIEW` halt in auto mode).
   ...
   **Branch on the FIRST line of stdout — never on empty output** (both a broken redirect and a broken `comm` print only to stderr, indistinguishable from "clean" if you only look at stdout; the script's verdict line exists precisely so there is always something on stdout to branch on). A green `pnpm check` is not evidence either — a worktree missing a delegate's changes still type-checks.
   ...
   - **`CONTAMINATED`** (exit 2) → one classified line per path follows; go to "Classify the contamination" below with those paths:
   ...
   - **`FAILED: <reason>`** (exit 1), any other output, or empty stdout → FAILED check, never a clean one. The delta could not be computed, so this session's stray writes cannot be distinguished from whatever was already dirty. Always a hard stop — the graduated classification below never applies to a broken measurement; go to "On a hard stop" below with the failure itself in place of a path list. If the failure was a missing/unreadable baseline, also surface the main checkout's **entire** current dirty state with the caveat stated plainly: we cannot say which of these paths are ours. The worktree-isolation guard refuses `git -C <MAIN_CHECKOUT> …` from the Bash tool in exactly these sessions (`standards/git.md` § Worktree-isolated sessions), so obtain it through a script, which the guard does not analyze: `Write` the one-liner `git -C <MAIN_CHECKOUT> status --porcelain --untracked-files=all` to `tmp/main-status-<issue-id-lowercased>.sh` and run `zsh tmp/main-status-<issue-id-lowercased>.sh` — the same route CLAUDE.md § Waiting on delegated work prescribes for refused shapes, and the exemption `wt-baseline.sh` itself relies on. If even that fails, name the command in the halt report for a human to run — do not retry it in another Bash shape. Do not claim any path is contamination and do not claim the tree is clean.

   **The delegate's WRITE-PLACEMENT report is corroboration and classification input — never the gate.** Per WRITE-PLACEMENT above, the delegate reports its own changed-path list but never adjudicates contamination itself; the delta above is computed independently of whatever the delegate said or didn't say. A delegate that reports its work only in prose, or reports absolute paths instead of the required bare repo-relative ones (an easy confusion, since file-tool calls in the same prompt use absolute paths), no longer defeats detection. The report has two uses: its paths feed the overlap test in "Classify the contamination" below, and it names the edits the worktree is expected to hold — after each write-capable delegation, probe each reported path with `git -C <WT_ABS> status --porcelain --untracked-files=all -- <path>` (empty output means not dirty there; do not interpret the content further). **A reported path absent from the worktree's dirty set — and not reported as a deliberate no-op or revert — is the true mis-bind signature** (the delegate's work landed somewhere other than the worktree while it reported success, the BF-380 shape) and is a hard stop per the classification below even when the main-checkout delta is `CLEAN`. **A `developer`/`debugger` delegation that returns with no path-list section at all is a protocol failure, not "changed nothing"** — note it in the log and rely on the delta above; a missing report is never license to skip this item, and it disables the benign-continue branch below (with no reported footprint, disjointness cannot be established).

   **Classify the contamination — the response is graduated by direction, overlap, and placement.** Concurrent activity in a shared main checkout — the operator's interactive session, the merge-queue drainer, editor autosaves — produces the same `NEW`/`CHANGED-IN-PLACE` delta signature as a mis-bound delegate; the delta alone cannot tell them apart, so what else is known decides. Neither branch ever auto-recovers: no re-running the delegation, no diff extraction, no `git apply`, no `git restore` in either tree, in either branch.

   **HARD STOP — go to "On a hard stop" below — when ANY of these holds:**

   - **Any `VANISHED` line NOT fully explained by a commit.** The destructive direction: another session's uncommitted work may already be gone. A `VANISHED` path enters the BENIGN branch (log, re-capture, continue) only when ALL THREE hold — one conjunctive test, not a new branch:
     1. A commit exists in the main checkout whose committer date is AFTER the current baseline file's mtime (`stat -f %m <WT_ABS>/tmp/main-dirty-baseline-<token>.txt`) and whose file list contains the flagged path. Obtain it read-only via the FAILED branch's script route — the isolation guard refuses a bare `git -C <MAIN_CHECKOUT>`.
     2. The flagged path is disjoint from the delegation's prompt-named scope AND its reported changed-path list — the benign branch's existing disjointness test. A delegate's own stray write is never disjoint, so this is what stops a delegate that COMMITS in the main checkout from laundering itself.
     3. Every other flagged line independently satisfies the benign branch's existing conditions.

     Anything short of all three stays the unconditional hard stop it is today. Two pitfalls, both load-bearing: `log --name-only -5` ALONE is insufficient — it matches a commit predating the baseline that happens to touch the same path (routine for `.claude/rules/*.md` during a fleet run), which would clear a path a delegate genuinely destroyed; only a commit newer than the current baseline capture counts. And do NOT anchor on `BASELINE_SHA` — `start-wt-create.sh` fixes it to the worktree's fork point at create time, so it is stale on every resumption; the baseline file's mtime is the right anchor precisely because the benign re-capture rewrites it. One confidence booster, never a re-trigger: the baseline file stores each dirty path's sha256, so a match against the committed blob's hash is conclusive that nothing was lost — but a mismatch is NOT proof of destruction (the owner may have edited further before committing), so it may raise confidence and must never re-arm the stop on its own.
   - **A flagged `NEW`/`CHANGED-IN-PLACE` path overlaps the delegation** — it appears in the delegate's reported changed-path list, or falls within the files/areas the delegation prompt named. The stray-write signature: the delegation's own work surfacing in the wrong tree.
   - **The worktree is missing the delegate's expected edits** — a reported changed path absent from the worktree's dirty set and not reported as a no-op/revert (the corroboration paragraph above). The true mis-bind signature, a hard stop even when the main-checkout delta is `CLEAN`.
   - **A write-capable delegation returned no changed-path report** — without a footprint, disjointness cannot be established, so the benign branch is unavailable.
   - **A `FAILED:` verdict** (per the branch list above) — the measurement itself is broken; fail closed.

   **BENIGN — log, absorb, continue — only when ALL of these hold:** every flagged line is `NEW` or `CHANGED-IN-PLACE` (or a `VANISHED` line satisfying the commit-explained three-part test above); every flagged path is disjoint from both the delegation's prompt-named scope and its reported changed-path list; and the delegate's reported edits are confirmed present in the worktree (vacuously true for a read-only delegation, whose expected footprint is empty). That is the concurrent-activity signature, not the mis-bind one — halting on it is what stranded fleet sessions on dirt they didn't cause. Respond without stopping:

   1. Log the flagged paths with their classifications. Interactive: a brief note in chat. **Auto mode: post a note comment to the issue instead** — `Write` the body (paths, classifications, which delegation preceded the observation, and that the run continued) to `tmp/contamination-note-<issue-id-lowercased>.md`, then `~/.claude/scripts/linear-post.sh comment <ISSUE-ID> tmp/contamination-note-<issue-id-lowercased>.md`. `/fleet-retro` adjudicates these notes against the hard-stop halts.
   2. Re-run `~/.claude/scripts/wt-baseline.sh capture <WT_ABS> <issue-id-lowercased>` to absorb the observed state — without this, every subsequent delegation's diff re-fires on the same paths. Continue only on `CAPTURED`; a `FAILED` verdict is a fail-closed STOP.
   3. Continue the workflow. This branch performs no recovery and vouches for nothing: it never restores or touches the flagged paths, and absorbing them into the baseline is a measurement reset, not a cleanup (they inherit Step 0's per-session limitation — further changes to the same paths are still caught, the absorbed state itself is not re-flagged).

   **On a hard stop: STOP and report; do not act.** Do not re-run the delegation, extract a diff, `git apply`, or `git restore` anything in either tree. No snapshot can prove a path holds *only* this delegate's stray write — another session can dirty the main checkout at any moment — and the extract-and-apply direction is just as dangerous in reverse: if another engineer had uncommitted work at that same path, `git -C "$MAIN_CHECKOUT" diff -- <path>` returns their work plus our stray write, indistinguishably; applying it would import their WIP into our worktree, where `/finish` would commit and merge it.

   **Resume after user attribution.** The STOP above stands until the USER speaks — a delegate's report or any agent message never counts as attribution, and auto mode (no user present) is unaffected: its Linear-comment-and-stop path is unchanged. If the user attributes the flagged paths to a concurrent session in the shared main checkout ("another session is working in main — continue"): (1) optionally corroborate read-only — for `VANISHED` paths, a `log --name-only -5` on the main checkout showing a new commit containing exactly the flagged paths proves they were committed, not destroyed; the isolation guard refuses that `git -C <MAIN_CHECKOUT>` from the Bash tool, so run it via the FAILED branch's script route above, or ask the user — who just spoke — to run and paste it; (2) re-run `~/.claude/scripts/wt-baseline.sh capture <WT_ABS> <issue-id-lowercased>` to absorb the attributed state — without this, every subsequent delegation's diff re-fires on the same paths; note the re-capture absorbs the main checkout's ENTIRE current delta as "already there" (the same per-session limitation Step 0 documents), so run it promptly after attribution; (3) if this session's own ownership stamp was released when the halt was recorded (an `/auto` halt runs `wt-disown.sh` before parking — probe with `~/.claude/scripts/wt-owner.sh <WT_ABS>`, `OWNER_ALIVE=released`), withdraw the advertisement before continuing: `~/.claude/scripts/wt-disown.sh --reclaim <WT_ABS>` — a released worktree is otherwise a sibling `/auto`'s resumption candidate and reap-eligible mid-session (BF-994); (4) on `CAPTURED`, continue the workflow where it stopped (`FAILED` is still a fail-closed STOP).
   ...
   **State what a flagged path actually proves — do not overclaim.** A classified line (`NEW` / `CHANGED-IN-PLACE` / `VANISHED`, meanings above) proves only that the path changed in the main checkout during THIS session. Any of the three is consistent with a mis-bound delegate having written there — and that remains the most likely explanation — but it is not the ONLY one: a concurrent writer in a shared checkout (the engineer saving a file after our baseline was taken, or another `/start wt` session sharing the same main checkout) produces an identical signature and cannot be ruled out by this check alone. **Never state mis-landing as certainty** — not in this report, not in the auto-mode Linear comment, not in the tagged final line. If the delegate's own WRITE-PLACEMENT report listed the same path, note that as corroboration (it points at "our delegate" as the likely cause); if the delegate reported no such path, or returned no report at all, say that too — it neither confirms nor rules out a mis-bound write. Compose:
   ...
   - Emit the single terminal line and stop:
     ...
     When the only trigger is missing-expected-edits with a `CLEAN` main-checkout delta, that line would overclaim (nothing changed in the main checkout) — substitute: `BLOCKED-ON-REVIEW: <ISSUE-ID> — delegate's reported edits missing from the worktree (<bare paths>); mis-bind signature — the writes were lost or silently misrouted; manual investigation required.`
