...
**Auto mode: skip the plan-mode tools entirely.** There is no user present to approve, so do NOT call `EnterPlanMode`/`ExitPlanMode` (an unanswered approval prompt would stall the loop). Do the same planning work — items 1–5 of the list below — compose the plan in the Step 7 comment format, and proceed **directly to Step 7**: the plan posted to Linear is the audit record a human reviews after the fact. Everything below this paragraph is interactive mode only.
...
4. Synthesize the exploration results into relevant files, patterns, and dependencies — dispatch a follow-up `Explore` only for genuine gaps
5. Design a step-by-step implementation plan
6. Identify which tasks are independent (parallelizable) vs dependent (sequential)
7. Write the plan to the plan file specified in the plan-mode system message
