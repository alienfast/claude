...
Biome rule inspects call sites for inherited defaults — its 22 parameter-adjacent rules all constrain the
declaration site. The caller that should have changed keeps compiling, with the behavior you just decided
was incorrect.
...
Measured on JA-317: a halt-message selector shipped as `options?: { provenPerDraft?: boolean }` defaulting
...
on it. Measured on JA-359, where a fix dispatch instructed that wrapping `outstanding: number` in an
